...
import { roothttp, http } from '@/utils'
...
const Article = () => {
  ...
  // 获取文章列表
  useEffect(() => {
    const rootloadList = async () => {
      const res = await roothttp.get('/root/articles', { params })
    ...
    const loadList = async () => {
      const res = await http.get('/content/articles', { params })
      const { results, total_count } = res.data
      setArticleData({
        list: results,
        count: total_count
  ...
  const delArticle = async (data) => {
    if (pathname === '/root/article_root') {
      await roothttp.delete(`/root/articles/${data.id}`)
    } else {
      await http.delete(`/content/articles/${data.id}`)
    }
  ...
  const goPublish = (data) => {
    if (pathname === '/root/article_root') {
      navigate(`/root/publish?id=${data.id}`)
    } else {
